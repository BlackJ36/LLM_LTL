...
class MujocoEnv(metaclass=EnvMeta):
    ...
    def reset(self):
        ...
        """
        if self.hard_reset is True:
            hard_reset_mode = True
        elif self.hard_reset is False:
            hard_reset_mode = False
        elif isinstance(self.hard_reset, float):
            assert 0.0 <= self.hard_reset <= 1.0
            if np.random.uniform() < self.hard_reset:
                hard_reset_mode = True
            else:
                hard_reset_mode = False
        else:
        ...
        if hard_reset_mode and not self.deterministic_reset:
    ...
    def step(self, action, num_sim_steps=None, render=False):
        ...
        if num_sim_steps is None:
            num_sim_steps = int(self.control_timestep / self.model_timestep)

        # Loop through the simulation at the model timestep rate until we're ready to take the next policy step
        # (as defined by the control frequency specified at the environment level)
        for i in range(num_sim_steps):
            # Note: forward() is called inside _pre_action -> controller.update(), no need to call it here
