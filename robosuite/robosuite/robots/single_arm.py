...
class SingleArm(Manipulator):
    ...
    def control(self, action, policy_step=False):
        ...
        if policy_step:
            pos_is_delta, ori_is_delta = self.skill_controller.ac_is_delta()
            set_pos = None
            set_ori = None
    ...
    def action_limits(self):
        ...
        """
        # Cache action limits since they don't change after initialization
        if not hasattr(self, '_cached_action_limits'):
            ...
            low = np.concatenate([low_c, low_g])
            high = np.concatenate([high_c, high_g])
            self._cached_action_limits = (low, high)

        return self._cached_action_limits
