...
class Controller(object, metaclass=abc.ABCMeta):
    ...
    def update(self, force=False):
        ...
        if self.new_update or force:
            ...
            # Cache site_id to avoid repeated lookups
            if not hasattr(self, '_eef_site_id'):
                self._eef_site_id = self.sim.model.site_name2id(self.eef_name)

            self.ee_pos = np.array(self.sim.data.site_xpos[self._eef_site_id])
            self.ee_ori_mat = np.array(self.sim.data.site_xmat[self._eef_site_id].reshape([3, 3]))
            # Use optimized single-site velocity computation (avoids computing ALL sites)
            self.ee_pos_vel, self.ee_ori_vel = self.sim.data.get_site_xvel_single(self._eef_site_id)

            self.joint_pos = np.array(self.sim.data.qpos[self.qpos_index])
            self.joint_vel = np.array(self.sim.data.qvel[self.qvel_index])

            # Use optimized combined Jacobian computation (avoids calling mj_jacSite twice)
            jacp, jacr = self.sim.data.get_site_jac_both(self.eef_name)
            self.J_pos = jacp.reshape((3, -1))[:, self.qvel_index]
            self.J_ori = jacr.reshape((3, -1))[:, self.qvel_index]
    ...
    def get_global_euler_from_ori_ac(self, ori_ac):
        """
        Converts orientation action to global euler angles.
        Default implementation returns zeros (for non-OSC controllers).
        OSC controller overrides this with proper implementation.

        Args:
            ori_ac: Orientation action

        Returns:
            np.array: Euler angles (default: zeros)
        """
        return np.zeros(3)
