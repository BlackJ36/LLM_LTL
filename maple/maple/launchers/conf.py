...
if maple.__file__ is not None:
    maple_project_dir = join(os.path.dirname(maple.__file__), os.pardir)
else:
    # Namespace package - use __path__
    maple_project_dir = join(list(maple.__path__)[0], os.pardir)

if robosuite.__file__ is not None:
    robosuite_project_dir = join(os.path.dirname(robosuite.__file__), os.pardir)
else:
    robosuite_project_dir = join(list(robosuite.__path__)[0], os.pardir)
...
try:
    from maple.launchers.conf_private import *
except ImportError:
