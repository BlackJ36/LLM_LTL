"""
Vectorized training script for MAPLE.

Uses multiple parallel environments for faster exploration sampling.
Gradients accumulate to the same model, accelerating single-task convergence.

Usage:
    uv run python maple/scripts/train_vectorized.py --env lift --num_envs 4
    uv run python maple/scripts/train_vectorized.py --env lift --num_envs 4 --debug
"""
import argparse
import collections
...
base_variant = dict(
    ...
    # Vectorized training settings (will be overridden by args)
    num_expl_envs=1,
    num_eval_envs=1,
    use_dummy_vec_env=False,
)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Vectorized MAPLE training with parallel environment sampling"
    )
    parser.add_argument('--env', type=str, required=True,
                        help='Environment name (lift, door, pnp, wipe, stack, nut, cleanup, peg_ins)')
    parser.add_argument('--num_envs', type=int, default=4,
                        help='Number of parallel exploration environments (default: 4)')
    parser.add_argument('--label', type=str, default='vec_test',
    ...
    parser.add_argument('--dummy_vec_env', action='store_true',
    ...
    parser.add_argument('--first_variant', action='store_true',
                        help='Only run first variant')
    parser.add_argument('--snapshot_gap', type=int, default=25,
                        help='Epochs between snapshots')

    args = parser.parse_args()

    print(f"=== Vectorized MAPLE Training ===")
    print(f"Environment: {args.env}")
    print(f"Parallel expl envs: {args.num_envs}")
    print(f"Parallel eval envs: {args.num_eval_envs}")
    print(f"Vec env type: {'DummyVecEnv' if args.dummy_vec_env else 'SubprocVecEnv'}")
